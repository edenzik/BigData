...
mahout/bin/mahout clusterdump -d filtered_tfidf/l2_${NGRAM}gram/dictionary.file-0 -dt sequencefile -i clustering_output/$KMEANS_PATH/clusters-*-final -n 10 -b 100 -o clusterdump_output/${KMEANS_PATH}.out 2>&1 | tee -a clusterdump_output/logs/$KMEANS_PATH.log
...
mahout/bin/mahout clusterdump -d filtered_tfidf/l2_${NGRAM}gram/dictionary.file-0 -dt sequencefile -i clustering_output/${FKMEANS_PATH}/clusters-*-final -n 10 -b 100 -o clusterdump_output/${FKMEANS_PATH}.out 2>&1 | tee -a clusterdump_output/logs/$FKMEANS_PATH.log
